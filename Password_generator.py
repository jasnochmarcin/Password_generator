...
def update_characterrs_left(number_of_characters):
    global characters_left

    if number_of_characters < 0 or number_of_characters > characters_left:
        print("Liczba znaków przekracza liczbę pozostałych znaków, tj.", characters_left)
        sys.exit(0)
    else:
...
if password_lenght < 6:
    print("Hasło jest za krótkie. Hasło musi mieć minimum 6 znaków, spróbuj jeszcze raz")
    sys.exit(0)
else:
    characters_left = password_lenght

lowercase_letters = int(input("Ile małych liter ma mieć hasło? "))
update_characterrs_left(lowercase_letters)

uppercase_letters = int(input("Ile dużych liter ma mieć hasło? "))
update_characterrs_left(uppercase_letters)

special_characters = int(input("Ile znaków specjalnych ma mieć hasło? "))
update_characterrs_left(special_characters)

digits = int(input("Ile cyfr ma mieć hasło?"))
update_characterrs_left(digits)

if characters_left > 0:
    print("Nie wykorzystałeś wszystkich znaków, uzupełniam hasło małymi literami")
